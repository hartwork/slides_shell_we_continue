...
Man _kann_ den nächsten https://github.com/cemeyer/httpd.sh[Webserver in Bash] schreiben, _sollte_ aber nicht unbedingt.
...
Hat einen Zustand, offene Dateien, belegt zeitweise CPU und Speicher.
...
# kill ${pid}
------------------------
Relevante Buitlins: `jobs`, `fg`, `bg`


== Typen: Was gibt es noch außer Variablen?, Funktionen, mkcd, Leerzeichen
...
`for i in .... ; do echo mv "${i}" "${i%.log}.txt" ; done`
...
`set -x` oder `set -o xtrace`

=== Rechner herunterfahren (wenn etwas fertig ist)
`sudo sh -c 'while pgrep emerge ; do sleep 1 ; done ; poweroff'`
...
=== Prozesse finden (mit Details)
`ps aux | fgrep ........`

=== Über welche Dienste bin ich per Netzwerk angreifbar?
`sudo netstat -tulpen`

=== Was für lokale IP-Adresse hat mein System?
`ip addr`
...
== !
Diese Folien sind entstanden mit Hilfe von:
...
* Hardlinks, Softlinks
* `dd`, `rsync`, `ssh`, `tmux`, `htop`, `strace`, `git`
* `lsblk`, `findmnt`
* `losetup`, `chroot`, `deboostrap`, `xsel`, `optipng`, `pv`, `ack`
